...
def depts_pick():
    parts="&".join(['IF(IFERROR(INDEX(\'Visit Log\'!$'+L(f"Dept: {d}")+'$2:$'+L(f"Dept: {d}")+f'${MAXROW},MATCH({PICK},{IDR},0)),"")="Yes",", {d}","")' for d in DEPTS])
    return f'=IF(({parts})="","",MID({parts},3,300))'
...
order=["Instructions","Visit Form","Visit Log"]
wb._sheets.sort(key=lambda s: order.index(s.title))
wb.active=order.index("Visit Form")

out="479_Field_Visit_Form.xlsx"; wb.save(out)
